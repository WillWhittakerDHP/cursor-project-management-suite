/**
 * Atomic Command: /check-reuse [description]
 ...
 * 
 * @deprecated - Migrated to planning abstraction. See `.cursor/commands/planning/atomic/check-reuse.ts`
...
export async function checkReuse(description: string): Promise<string> {
  ...
  output.push('- `client-vue/src/api/transformers/adminTransformer.ts` - Vue admin transformer');
  output.push('- `client-vue/src/api/transformers/globalTransformer.ts` - Global transformer pattern\n');
...
async function searchForPatterns(description: string): Promise<ReuseCheckResult> {
  ...
  try {
    const transformerDirs = [
      ...
      join(PROJECT_ROOT, 'client-vue/src/api/transformers')
    ];
