...
function detectFilesFromGitHistory(phase: string, featureName: string): string[] {
  try {
    ...
    const files = output
      ...
      // Filter to relevant paths (client-vue, server, or .cursor commands)
      .filter(file => 
        file.startsWith('client-vue/') ||
...
function handleFileRenames(files: string[]): string[] {
  ...
  const fileRenameMap: Record<string, string> = {
    ...
    'client-vue/src/constants/properties.ts': 'client-vue/src/constants/primitives.ts',
  };
