...
export class FeatureContext {
   ...
   * 
   * Detection order:
   * 1. Check `.cursor/project-manager/.current-feature` config file
  ...
  static async getCurrent(): Promise<FeatureContext> {
    ...
    const configPath = join(PROJECT_ROOT, '.cursor/project-manager/.current-feature');
    ...
    try {
      ...
      throw new Error(
        ...
        `Please specify feature name explicitly or set .cursor/project-manager/.current-feature config file.`
      );
    }
  ...
  static async listAvailableFeatures(): Promise<string[]> {
    const PROJECT_ROOT = process.cwd();
    const featuresPath = join(PROJECT_ROOT, '.cursor/project-manager/features');
