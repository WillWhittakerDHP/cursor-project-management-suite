  ...
  if (changeRequest.type === 'naming' && changeRequest.oldValue) {
    try {
      ...
      const searchResult = execSync(
        `grep -r --include="*.ts" --include="*.tsx" --include="*.vue" --include="*.js" --include="*.jsx" -l "${changeRequest.oldValue}" client-vue/ client/ 2>/dev/null || true`,
 ...
 */
export function enforcePlanMode(commandName: string): PlanModeEnforcementResult {
