...
export async function readHandoff(
  ...
  if (tier === 'phase' && identifier) {
    try {
      ...
      attemptedPath = context.paths.getPhaseHandoffPath(identifier);
    } catch (error) {
      throw new Error(
    ...
    if (!WorkflowId.isValidSessionId(identifier)) {
      throw new Error(`ERROR: Invalid session ID format. Expected X.Y (e.g., 1.3)\nAttempted: ${identifier}`);
    }
    try {
      content = await context.readSessionHandoff(identifier);
      attemptedPath = context.paths.getSessionHandoffPath(identifier);
    } catch (error) {
      throw new Error(
    ...
    if (!parsedTask) {
      throw new Error(`ERROR: Invalid task ID format. Expected X.Y.Z (e.g., 1.3.1)\nAttempted: ${identifier}`);
    }
    const sessionId = WorkflowId.generateSessionId(parsedTask.phase, parsedTask.session);
    try {
      content = await context.readSessionHandoff(sessionId);
      attemptedPath = context.paths.getSessionHandoffPath(sessionId);
    } catch (error) {
      throw new Error(
    ...
    try {
      ...
      attemptedPath = context.paths.getFeatureHandoffPath();
    } catch (error) {
      throw new Error(
