...
- **Start/end:** The command includes the tier identifier. Entry calls `contextFromParams(config.name, params)` and passes context to the adapter and impls. **Phase, session, and task** params **must** include **`featureId` or `featureName`** (PROJECT_PLAN `#` or directory slug); resolution is via **`resolveWorkflowScope`** only (no git-branch inference). See `.project-manager/HARNESS_CHARTER.md` and this playbook (this section and **Tier navigation** below).
