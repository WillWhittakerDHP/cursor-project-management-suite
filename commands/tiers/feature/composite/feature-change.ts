...
export async function featureChange(featureName: string, newFeatureName: string, reason: string): Promise<string> {
  ...
  try {
    const featureLogPath = `.cursor/project-manager/features/${featureName}/feature-${featureName}-log.md`;
  ...
  try {
    const changeDocPath = `.cursor/project-manager/features/${featureName}/feature-${featureName}-to-${newFeatureName}-change.md`;
    const changeDocContent = `# Feature Change: ${featureName} → ${newFeatureName}\n\n**Date:** ${new Date().toISOString().split('T')[0]}\n**Reason:** ${reason}\n\n## Context\n\n[Current feature state and reason for change]\n\n## Transition Plan\n\n[How to transition from old feature to new feature]\n\n## Impact Assessment\n\n[Code, documentation, and testing impacts]\n\n## Risk Mitigation\n\n[Potential risks and mitigation strategies]\n\n## Timeline\n\n[Estimated duration and milestones]\n\n---\n\n## Related Documents\n\n- Feature Guide: \`.cursor/project-manager/features/${featureName}/feature-${featureName}-guide.md\`\n- Feature Log: \`.cursor/project-manager/features/${featureName}/feature-${featureName}-log.md\`\n- New Feature Guide: \`.cursor/project-manager/features/${newFeatureName}/feature-${newFeatureName}-guide.md\`\n`;
