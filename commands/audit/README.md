...
- `auditSession` - All 7 audits for session tier
- `auditPhase` - All 7 audits for phase tier
- `auditFeature` - All 7 audits for feature tier
- `auditTask` - 5 audits for task tier (excludes planning and docs)
...
const sessionAudit = await auditSession({
  ...
  modifiedFiles: ['client-vue/src/composables/useEntity.ts'],
