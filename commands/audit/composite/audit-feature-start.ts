...
export interface AuditFeatureStartParams {
  featureName: string;
}
...
export async function auditFeatureStart(params: AuditFeatureStartParams): Promise<{
  ...
  outputLines.push(`# Feature Start Audit: ${params.featureName}`);
  outputLines.push('');
  outputLines.push(`**Purpose:** Baseline quality assessment before feature work begins`);
